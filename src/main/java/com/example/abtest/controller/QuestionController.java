...
public class QuestionController {
  ...
  public String listQuestions(Model model) {
    // 초기 10개만 로드 (무한 스크롤로 나머지 로드)
    QuestionSliceResponse initialQuestions = questionService.getQuestionsWithPaging(0, 10);
    model.addAttribute("questions", initialQuestions.getContent());
    model.addAttribute("hasNext", initialQuestions.isHasNext());
  ...
  public String createQuestion(@ModelAttribute QuestionForm form,
      ...
      @RequestParam(value = "imageB", required = false) MultipartFile imageB) throws IOException {
  ...
  public String createAnswer(@PathVariable Long id, @RequestParam("answerText") String answerText,
